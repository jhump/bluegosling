...
/**
 * Utility methods and constants for using and creating {@code ObjectVerifier}
 * instances.
 * 
 * @author jhumphries
 */
public class ObjectVerifiers extends Assert {
   ...
   /**
    * An object verifier that doesn't actually do any verification but just returns
    * the test object.
    */
   ...
   /**
    * An object that verifies that the test object is null if the reference object
   ...
   /**
    * An object that verifies that the test object is equal to the reference object
    * by checking that their hash codes are the same. If one of the objects is null
    * and the other is non-null, verification will fail.
    */
   ...
   /**
    * An object that verifies that the test object is the same instance as the
    * reference object (via the == operator).
    */
   ...
   /**
    * An object that verifies that the test exception has the same class as that
    * of the reference exception. If one of the objects is null and the other is
    * non-null, verification will fail.
    */
   ...
   /**
    * Returns an object that verifies that the test exception is <em>compatible</em>
    * with the reference exception.
    * 
    * <p>The test exception is compatible if any of the following are true:
    * <ul>
    * <li>The test exception's class is the same as that of the reference
    * exception</li>
    * <li>The test exception's class is a sub-class of that of the reference
    * exception</li>
    * <li>Both the test exception and reference exception are sub-classes of
    * one of the specified exception classes</li>
    * </ul>
    * 
    * <p>If one of the objects is null and the other is non-null, verification will fail.
    * 
    ...
    */
   public static ObjectVerifier<Throwable> relaxedExceptions(Set<Class<? extends Throwable>> exceptions) {
      // check inputs
      if (exceptions == null) {
      ...
      final Set<Class<? extends Throwable>> myCopy = new HashSet<Class<? extends Throwable>>(exceptions);
    ...
    */
   //@SafeVarargs
   public static ObjectVerifier<Throwable> relaxedExceptions(Class<? extends Throwable>... exceptions) {
   ...
   /**
    * Returns an object that verifies that the test object matches the
    * reference object by using the specified {@code Comparator}. The object
    * will verify that {@link Comparator#compare(Object, Object)} returns
    * zero when comparing the test and reference objects. If one of the
   ...
   /**
    * Returns an object that performs all specified verifications. This verifier
    * will return the value returned by the <em>last</em> verifier provided.
    * 
    * @param <T> the type of object being verified
