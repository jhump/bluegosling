...
/**
 * Provides a framework for implement blocking locks and related synchronizer that rely on FIFO
...
public class AbstractQueuedReferenceSynchronizer<S, R> extends AbstractOwnableSynchronizer {
    ...
    *
    * <p>The wait queue is a variant of a "CLH" (Craig, Landin, and Hagersten) lock queue. CLH
    * locks are normally used for spinlocks. We instead use them for blocking synchronizers, but
    * use the same basic tactic of holding some of the control information about a thread in the
    * predecessor of its node. A "status" field in each node keeps track of whether a thread should
    * block. A node is signalled when its predecessor releases.  Each node of the queue otherwise
    * serves as a specific-notification-style monitor holding a single waiting thread. The status
    * field does NOT control whether threads are granted locks etc though. A thread may try to
    * acquire if it is first in the queue. But being first does not guarantee success; it only
    * gives the right to contend.  So the currently released contender thread may need to rewait.
    *
    * <p>To enqueue into a CLH lock, you atomically splice it in as new tail. To dequeue, you just
    * set the head field.
    * <pre>
    *      +------+  prev +-----+       +-----+
    * head |      | <---- |     | <---- |     |  tail
    ...
    * </pre>
    *
    * <p>Insertion into a CLH queue requires only a single atomic operation on "tail", so there is
    * a simple atomic point of demarcation from unqueued to queued. Similarly, dequeuing involves
    ...
    *
    * <p>The "prev" links (not used in original CLH locks), are mainly needed to handle
    * cancellation. If a node is cancelled, its successor is (normally) relinked to a non-cancelled
    * predecessor. For explanation of similar mechanics in the case of spin locks, see the
    * <a href="http://www.cs.rochester.edu/u/scott/synchronization/">papers by Scott and
    * Scherer</a>. 
    *
    * <p>We also use "next" links to implement blocking mechanics. The thread id for each node is
    * kept in its own node, so a predecessor signals the next node to wake up by traversing next
    * link to determine which thread it is. Determination of successor must avoid races with newly
    * queued nodes to set the "next" fields of their predecessors. This is solved when necessary by
    ...
    * a backward scan.)
    *
    * <p>Cancellation introduces some conservatism to the basic algorithms.  Since we must poll for
    ...
    *
    * <p>CLH queues need a dummy header node to get started. But we don't create them on
    * construction, because it would be wasted effort if there is never contention. Instead, the
    * node is constructed and head and tail pointers are set upon first contention.
    *
    * <p>Threads waiting on Conditions use the same nodes, but use an additional link. Conditions
    * only need to link nodes in simple (non-concurrent) linked queues because they are only
    * accessed when exclusively held.  Upon await, a node is inserted into a condition queue. Upon
    * signal, the node is transferred to the main queue. A special value of status field is used to
    * mark which queue a node is on.
    *
    * <p>Thanks go to Dave Dice, Mark Moir, Victor Luchangco, Bill Scherer and Michael Scott, along
   ...
   static final class Node {
      ...
      /** Wait status that indicates thread is waiting on condition. */
       ...
       * <dl>
       *  <dt>SIGNAL</dt>
       *  <dd>The successor of this node is (or will soon be) blocked (via park), so the current
       *  node must unpark its successor when it releases or cancels. To avoid races, acquire
       *  methods must first indicate they need a signal, then retry the atomic acquire, and then,
       *  on failure, block.</dd>
       *  <dt>CANCELLED</dt>
       ...
       * </dl>
       *
       * <p>The values are arranged numerically to simplify use. Non-negative values mean that a
       * node doesn't need to signal. So, most code doesn't need to check for particular values,
       * just for sign.
       *
       * <p>The field is initialized to 0 for normal sync nodes, and {@link #CONDITION} for
       * condition nodes. It is modified using CAS (or when possible, unconditional volatile
       * writes).
       */
   ...
   // TODO: customize serialization to initialize these transient fields
   
   /**
    * Head of the wait queue, lazily initialized. Except for initialization, it is modified only
    * via method setHead. Note: If head exists, its waitStatus is guaranteed not to be
    * {@link #CANCELLED}.
    */
    ...
    * equals the expected value. This operation has memory semantics of a {@code volatile} read
    * and write.
    *
   ...
   private void doReleaseShared() {
    ...
    private void setHeadAndPropagate(Node node, int propagate) {
        ...
        if (propagate > 0 || h == null || h.waitStatus.get() < 0 ||
            (h = head.get()) == null || h.waitStatus.get() < 0) {
            Node s = node.next.get();
     ...
     * @param arg the acquire argument
     */
    private void doAcquireInterruptibly(R arg)
     ...
     */
    private boolean doAcquireNanos(R arg, long nanosTimeout)
            throws InterruptedException {
        if (nanosTimeout <= 0L)
            return false;
     ...
     * @param arg the acquire argument
     */
    private void doAcquireSharedInterruptibly(R arg)
        throws InterruptedException {
        final Node node = addWaiter(Node.SHARED);
        boolean failed = true;
        try {
     ...
     * @return {@code true} if acquired
     */
    private boolean doAcquireSharedNanos(R arg, long nanosTimeout)
            throws InterruptedException {
        if (nanosTimeout <= 0L)
            return false;
        final long deadline = System.nanoTime() + nanosTimeout;
     ...
     */
    public final void acquireInterruptibly(R arg)
            throws InterruptedException {
        if (Thread.interrupted())
            throw new InterruptedException();
        if (!tryAcquire(arg))
     ...
     * @throws InterruptedException if the current thread is interrupted
     */
    public final boolean tryAcquireNanos(R arg, long nanosTimeout)
            throws InterruptedException {
        if (Thread.interrupted())
            throw new InterruptedException();
     ...
     * @throws InterruptedException if the current thread is interrupted
     */
    public final void acquireSharedInterruptibly(R arg)
            throws InterruptedException {
        if (Thread.interrupted())
            throw new InterruptedException();
        if (tryAcquireShared(arg) < 0)
     ...
     * @throws InterruptedException if the current thread is interrupted
     */
    public final boolean tryAcquireSharedNanos(R arg, long nanosTimeout)
            throws InterruptedException {
        if (Thread.interrupted())
            throw new InterruptedException();
    ...
    private Thread fullGetFirstQueuedThread() {
        ...
        if (((h = head.get()) != null && (s = h.next.get()) != null &&
             s.prev == head.get() && (st = s.thread) != null) ||
