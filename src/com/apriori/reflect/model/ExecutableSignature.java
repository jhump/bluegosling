...
class ExecutableSignature implements AnnotatedElement {
   ...
   public boolean isSameSignature(ExecutableSignature other) {
      ...
      if (typeParameters.length > 0) {
         ...
         for (int i = 0; i < typeParameters.length; i++) {
            ...
            for (int j = 0; j < bounds.length; j++) {
               Type b = com.apriori.reflect.Types.replaceTypeVariables(bounds[j],
                     adaptedTypeVariables);
               if (!com.apriori.reflect.Types.equals(b, otherBounds[j])) {
                  return false;
               }
            }
         }
         ...
         for (int i = 0; i < paramTypes.length; i++) {
            parameters[i] = com.apriori.reflect.Types.replaceTypeVariables(
      ...
      for (int i = 0; i < parameters.length; i++) {
         if (!com.apriori.reflect.Types.equals(parameters[i], other.paramTypes[i].getType())) {
            return false;
         }
      }
   ...
   private static boolean allEqual(Type[] t1, Type[] t2) {
      assert t1.length == t2.length;
      for (int i = 0; i < t1.length; i++) {
         if (!com.apriori.reflect.Types.equals(t1[i], t2[i])) {
            return false;
         }
      }
      return true;
