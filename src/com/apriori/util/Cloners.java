...
public class Cloners {
   ...
   public static void checkClone(Object orig, Object clone) {
      ...
      else if (clone.getClass() != orig.getClass()) { throw new CloningException(
            "Clone should be of the same class as original object"); }
   }

   /**
    * Returns a cloner for cloning instances of {@code Cloneable}. This uses the instance's
    * {@code clone()} method but furthermore checks that the resulting clone is the same class but
    * not the same instance as the source instance. So classes that implement {@code Cloneable}, but
    * in non-standard ways, may not be suitable and may cause exceptions to occur during cloning.
    * 
    * @param <T> The type of object cloned
    * @return A cloner that will use an object's {@code clone()} method to perform the cloning
    *         operation
    */
   ...
   /**
    * Returns a cloner for cloning instances of {@code Serializable}. This will actually serialize
    * and de-serialize the object to create the clone. It furthermore checks that the resulting
    * clone is the same class but not the same instance as the source instance. So classes that use
    * custom serialization via {@code writeReplace()} and/or {@code readResolve()} methods may not
    * be suitable.
    * 
    * @param <T> The type of object cloned
    * @return A cloner that will use object serialization and de-serialization to perform the
    *         cloning operation
    */
   ...
   /**
    * Interface that can look-up class members via reflection. This is generic because it is used
    * for both looking up constructors and methods.
    * 
    * @author Joshua Humphries (jhumphries131@gmail.com)
    * 
   ...
   private interface MemberGetter<T extends AccessibleObject> {
      /**
       * Retrieve a member that accepts the specified single argument type as a parameter.
       * 
   ...
   /**
    * Adds all interfaces declared by the specified class to the queue (for implementing the
    * breadth-first search in {@link #findMemberForInterface}).
    * 
    * @param clazz The class whose declared interfaces are added to the queue
    * @param level The level for the interfaces added
    * @param queue The queue
    */
   private static void addAllInterfacesToSearch(Class<?> clazz, int level,
         ArrayDeque<InterfaceSearchNode> queue) {
   ...
   /**
    * Finds a member that accepts an interface implemented by the specified class. This performs a
    * breadth-first search through the specified class's interfaces. The first "level" of the search
    * are the interfaces directly implemented by the class. The second level consists of
    * super-interfaces for those interfaces, the third level includes super-interfaces of those
    * super-interfaces, and so on.
    * 
    ...
    * @throws IllegalArgumentException If the search is ambiguous because multiple matches are found
    *            with the same level of "specificity" (same level in in the search)
    */
   private static <T extends AccessibleObject> T findMemberForInterface(Class<?> clazz,
         MemberGetter<T> getter) {
   ...
   /**
    * Finds the member (constructor or method) that accepts the most specific compatible argument
    * type for copying the specified class.
    * 
    * @param <T> The type of member (constructor or method) that we're looking for
    * @param clazz The class whose interfaces are searched
    * @param getter Object that can retrieve a matching member
    ...
    *            with the same level of "specificity" (same level in in the search)
    */
   private static <T extends AccessibleObject> T findMostSpecificMember(Class<?> clazz,
         MemberGetter<T> getter) {
      T ret = null;
   ...
   /**
    * Finds a copy constructor for the specified class. This requires that the class have a single
    * argument constructor that accepts a compatible argument type.
    * 
    * <p>
    * If there are multiple such copy constructors (like one that takes precisely the same type and
    * an alternate constructor whose parameter is a super-type) then the one with the
    * <em>most specific</em> type is returned. The class hierarchy is checked first, so a
    * constructor that accepts a super-class or ancestor class is preferred over constructors that
    * accept compatible interfaces, with an exception made for constructors that take {@code Object}
    * (which is the <em>least</em> specific). Next, interfaces declared on the class itself are
    * checked and then ancestors of the declared interfaces. Then interfaces declared on the
    * super-class are checked and so on and so forth.
    * 
    * <p>
    * If multiple constructors are found with the same level of "specificity" then an exception will
    * be thrown. This can happen, for example, when no constructor accepts the specified class or an
    * ancestor class and more than one constructor exists that each accept an interface declared on
    * the class.
    * 
    * <p>
    ...
    * </tr>
    * <tr>
    * <th rowspan=3>Ancestor classes...</th>
    * <td>{@code Class3}</td>
    * </tr>
    * <tr>
    * <td>{@code Class2}</td>
    ...
    * </tr>
    * <tr>
    * <th>Interfaces for {@code MyClass}</th>
    * <td>{@code Interface1}</td>
    * </tr>
    * <tr>
    * <th rowspan=2>Super-interfaces</th>
    * <td>{@code Interface1A}, {@code Interface1B}</td>
    * </tr>
    * <tr>
    * <td>{@code Interface1A1}</td>
    * </tr>
    * <tr>
    * <th rowspan=2>Interfaces for ancestor classes...</th>
    * <td>{@code Interface2}, {@code Interface3}</td>
    * </tr>
    * <tr>
    * <td>{@code Interface2A}, {@code Interface2B}</td>
    * </tr>
    * <tr>
    ...
    * {@code Interface1A} and one that took an {@code Interface1B}, then an exception would be
    * thrown because it is ambiguous which constructor should be used. If, however, the class also
    * had a constructor that took a {@code Class1} then that constructor would be used since it is
    * more specific than the other two.
    * 
    * @param <T> The type of object to be copied
    * @param clazz The class token for the type of object to be copied and the class that must have
    *           the copy constructor
    * @return The copy constructor or null if no compatible constructor exists
    * @throws IllegalArgumentException If the choice of constructor is ambiguous because there are
    *            multiple such constructors with the same level of specificity
   ...
   /**
    * Finds a static copy method for the specified class with the specified name. This requires that
    * the class have a single argument method with the specified name that accepts a compatible
    * argument type.
    * 
    * <p>
    * This will involve the same "specificity" semantics as {@link #findCopyConstructor(Class)} when
    * searching for the copy method.
    * 
    * @param <T> The type of object to be copied
    * @param clazz The class token for the type of object to be copied and the class that must have
    *           the copy method
    * @param methodName The name of the static copy method
    * @return The static copy method or null if no compatible method with the specified name exists
    * @throws IllegalArgumentException If the choice of method is ambiguous because there are
    *            multiple such methods with the same level of specificity
    * @throws NullPointerException If the specified class token is null
    */
   ...
   /**
    * Returns a cloner that uses the specified copy constructor to perform the cloning operation.
    * The constructor should take no arguments or take a single argument that is the object being
    * copied.
    * 
   ...
   public static <T> Cloner<T> withCopyConstructor(final Constructor<T> cons) {
      ...
      if (argTypes.length > 1) {
         ...
         else if (!argTypes[0].isAssignableFrom(cons.getDeclaringClass())) { throw new IllegalArgumentException(
   ...
   /**
    * Returns a cloner that uses a class's copy constructor to perform the cloning operation. This
    * requires that the class have a single-argument constructor that accepts a compatible type.
    * 
    * <p>
    * The copy constructor to use is determined by {@link #findCopyConstructor(Class)}. If multiple
    * constructors are found with the same level of "specificity" then an exception will be thrown
    * and {@link #withCopyConstructor(Class, Class)} or {@link #withCopyConstructor(Constructor)}
    * should be used instead.
    * 
    * @param <T> The type of object being cloned
    * @param clazz The class token for the type of object being cloned
    * @return A cloner that will use a copy constructor to create a clone
    * @throws IllegalArgumentException If the specified class has no suitable copy constructor or if
    *            it has multiple constructors of the same level specificity
    * @throws NullPointerException If the specified class token is null
   ...
   public static <T> Cloner<T> withCopyConstructor(Class<T> clazz) {
      ...
      if (cons == null) { throw new IllegalArgumentException("No copy constructor exists for "
            + clazz.getName()); }
   ...
   /**
    * Returns a cloner that uses a class's copy constructor to perform the cloning operation. This
    * requires that the class have a single-argument constructor that accepts the specified type.
    * 
    * @param <T> The type of object being cloned
    * @param clazz The class token for the type of object being cloned
    * @param argClazz The argument type of the copy constructor
    * @return A cloner that will use a copy constructor to create a clone
    * @throws IllegalArgumentException If the specified class has no matching constructor
   ...
   public static <T> Cloner<T> withCopyConstructor(Class<T> clazz, Class<? super T> argClazz) {
      if (clazz == null || argClazz == null) { throw new NullPointerException(); }
   ...
   /**
    * Returns a cloner that uses the specified copy method to perform the cloning operation. If the
    * method is a static method then it should take no arguments or take a single argument that is
    * the object being copied. If the method is an instance method then it should take no arguments
    * and the instance it is invoked on will be the object being copied.
    * 
    * @param <T> The type of object being cloned
    * @param method The method to use to create a clone
    * @return A cloner that will use the method to create a clone
    * @throws IllegalArgumentException If the specified method requires more than one argument or
    *            requires an argument that is a primitive type
   ...
   public static <T> Cloner<T> withCopyMethod(final Method method) {
      ...
      if (argTypes.length > 0 && !isStatic) {
         ...
         else if (!argTypes[0].isAssignableFrom(method.getDeclaringClass())) { throw new IllegalArgumentException(
   ...
   /**
    * Returns a cloner that uses an instance method on the class to perform the cloning operation.
    * This requires that the class have a no-argument method with the specified name that accepts a
    * compatible type. The instance method will be invoked on the object to be cloned.
    * 
    * @param <T> The type of object being cloned
    * @param clazz The class token for the type of object being cloned
    * @param methodName The name of the instance method
    * @return A cloner that will use an instance method to create a clone
   ...
   public static <T> Cloner<T> withInstanceCopyMethod(Class<T> clazz, String methodName) {
      if (clazz == null || methodName == null) { throw new NullPointerException(); }
      Class<?> currentClass = clazz;
      while (currentClass != null) {
         try {
            Method method = currentClass.getDeclaredMethod(methodName);
            int mods = method.getModifiers();
            if (Modifier.isStatic(mods)) { throw new IllegalArgumentException(
                  "No instance method named " + methodName
      ...
      // went through entire hierarchy w/out finding the method
      throw new IllegalArgumentException("No copy method named " + methodName
            + " exists for " + clazz.getName());
   }

   /**
    * Returns a cloner that uses a static method on the class to perform the cloning operation. This
    * requires that the class have a single-argument method with the specified name that accepts a
    * compatible type.
    * 
    * <p>
    * The copy method to use is determined by {@link #findStaticCopyMethod(Class, String)}. If
    * multiple matching methods are found with the same level of "specificity" then an exception
    * will be thrown and {@link #withStaticCopyMethod(Class, String, Class)} or
    * {@link #withCopyMethod(Method)} should be used instead.
    * 
    * @param <T> The type of object being cloned
    * @param clazz The class token for the type of object being cloned
    * @param methodName The name of the static method
    * @return A cloner that will use a static method to create a clone
    * @throws IllegalArgumentException If the specified class has no suitable static method or if it
    *            has multiple such methods of the same level specificity
    * @throws NullPointerException If either argument is null
   ...
   public static <T> Cloner<T> withStaticCopyMethod(Class<T> clazz, String methodName) {
      ...
      if (method == null) { throw new IllegalArgumentException("No copy method named " + methodName
               + " exists for " + clazz.getName()); }
      return withCopyMethod(method);
   }
   ...
   /**
    * Returns a cloner that uses a static method on the class to perform the cloning operation. This
    * requires that the class have a single-argument method with the specified name that accepts the
    * specified type.
    * 
    * @param <T> The type of object being cloned
    * @param clazz The class token for the type of object being cloned
    ...
    * @return A cloner that will use a static method to create a clone
    * @throws IllegalArgumentException If the specified class has no matching static method
    * @throws NullPointerException If any of the arguments are null
    */
   public static <T> Cloner<T> withStaticCopyMethod(Class<T> clazz, String methodName,
         Class<? super T> argClazz) {
      if (clazz == null || methodName == null || argClazz == null) { throw new NullPointerException(); }
   ...
   /**
    * Creates a cloner that just returns the specified object. This is useful when the clone is
    * created beforehand and needs to be embedded in a {@code Cloner} interface. The cloner will
    * perform a check that the specified clone object has the same class as the object to be cloned
    * and is not the same instance as the object being cloned.
    * 
    * @param <T> The type of object being cloned
   ...
   public static <T> Cloner<T> fromInstance(final T clone) {
      if (clone == null) { throw new NullPointerException(); }
      return new Cloner<T>() {
         @Override
         public T clone(T o) {
            // check the object's value and type
   ...
   /**
    * Creates a list of cloners that just return the specified objects. This is useful when the
    * clones are created beforehand and need to be embedded in {@code Cloner} interfaces. The
    * cloners will perform checks that the specified clone objects have the same classes as the
    * objects to be cloned and are not the same instances as the objects being cloned.
    * 
    ...
    * 
    * <p>
    * The returned list will have cloners in the same order as the specified clones. So the first
    * cloner in the list will return the first item from the provided list of clones.
    * 
    * <p>
   ...
   public static List<Cloner<?>> fromInstances(List<?> clones) {
      if (clones == null) { throw new NullPointerException(); }
   ...
   /**
    * Creates a cloner that gets the clone from the specified object. Since {@link Callable#call()}
    * does not take any parameters, the clone must be agnostic to the actual value being cloned.
    * From that perspective, this is similar to using {@link #fromInstance(Object)}.
    * 
    * @param <T> The type of object being cloned
    * @param c The object called to get the clone
    * @return A cloner that calls the specified object and returns the result as the clone
    * @throws NullPointerException If the specified instance is null
    * 
    ...
    * elements. If you want a cloner that does a <em>shallow</em> copy of the array, you can use
    * {@link #forCloneable()} since arrays in Java implement {@code Cloneable}.
    * 
      ...
      return new Cloner<T[]>() {
         ...
         public T[] clone(T[] o) {
            ...
            for (int i = 0; i < o.length; i++) {
               // clone all of the non-null elements
               if (o[i] != null) {
   ...
   /**
    * Creates a cloner for arrays that performs a deep copy by cloning all array elements. The
    * "leaf" components (not arrays) will be cloned using the specified cloner. The rest of the
    * structure, regardless of nesting depth, will be deep copied in the resulting clone.
    * 
    * This can be useful if you have a cloner for {@code SpreadsheetCell} and a sheet of cells
    * represented by a two-dimensional array of these objects: {@code SpreadsheetCell[][]}. In such
    * a case, a cloner for this structure could be build like so:
    * 
    * <pre>
    ...
    * </pre>
    * 
    * This is effectively shorthand for repeated calls to {@link #forArray(Cloner)} that look like
    * this:
    * 
    * <pre>
    * Cloner&lt;SpreadsheetCell&gt; cellCloner = new MyCellCloner();
    ...
    * </pre>
    * 
    * And this method works, regardless of nesting depth. So the following call is just as valid and
    * will create a cloner for a four-dimensional array (and will save even more typing since using
    * {@code forArray} with a four-dimensional array is even more verbose):
    * 
    * <pre>
    ...
    * </pre>
    * 
    * @param <T> The component type of the array type to clone (which can itself be an array type
    *           since this method is designed to work with nested arrays)
   ...
   /**
    * Creates a "default" cloner for the specified class. The default cloner is chosen like so:
    * <ul>
    * <li>If the type represents an array then {@link Cloners#forArray(Cloner)} is used to create a
    * cloner that <em>deep copies</em> arrays. Elements in the array will be cloned by a "default"
    * cloner for their type.
    * <li>If the type implements {@code Cloneable} then {@link Cloners#forCloneable()} is used to
    * create the cloner.</li>
    * <li>If the type implements {@code Serializable} then {@link Cloners#forSerializable()} is
    * used.</li>
    * <li>Otherwise, {@link Cloners#withCopyConstructor(Class)} is used to create the cloner.
    * </ul>
    * If the type is not an array, implements neither interface, and has no suitable copy
    * constructor then an exception is thrown. Similarly, if the type is an array but has an element
    * type that implements neither interface and has no suitable copy constructor then an exception
    * is thrown.
    * 
   ...
   /**
    * A generic cloner that will try to clone any type of object. Its strategy for cloning objects
