 ...
 * typesafe interface for typical operations.
 * 
 * <p>This interface includes methods provided by every tuple, regardless of the number of items.
 * Additional interfaces are also provided for tuples with up to five items that provide typesafe
 * operations on the items contained in the tuple.
 * 
 * <p>Implementations of this interface offer typesafe operations for instances with up to five
 * items. Tuples with more than five items have less rich operations for the sixth and later items
 * and also do not have the same level of type safety. If you need tuples with more than five items,
 * consider using a {@link List} instead.
 *
...
public interface Tuple extends Iterable<Object> {
   ...
   /**
    * Adds an item to the existing item of this tuple. The item will be appended and thus ordered
    * last in the new tuple.
    * 
    ...
    * first item in this tuple is the second item in the returned tuple.
    * 
    * @param t the item to insert
    * @return a new tuple that is the specified item followed by the item in this tuple
    */   
   <T> Ops1<T> insertFirst(T t);
   
   /**
    * Transforms every item in this tuple by applying the specified function to each one. The 
    * returned tuple will have the same number of items as this tuple.
    * 
