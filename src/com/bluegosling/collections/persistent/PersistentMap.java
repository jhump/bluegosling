...
public interface PersistentMap<K, V> extends Map<K, V> {
   ...
   default PersistentMap<K, V> withComputed(K key, BiFunction<? super K, ? super V, ? extends V> fn) {
      V newValue = fn.apply(key, get(key));
      return newValue == null ? withoutKey(key) : with(key, newValue);
   }

   default PersistentMap<K, V> withComputedIfAbsent(K key, Function<? super K, ? extends V> fn) {
      if (containsKey(key)) {
         return this;
      }
   ...
   default PersistentMap<K, V> withComputedIfPresent(K key,
   ...
   default PersistentMap<K, V> withMerged(K key, V value,
