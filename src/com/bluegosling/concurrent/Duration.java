...
public class Duration implements Comparable<Duration> {
   ...
   public static void main(String[] args) {
      ...
      for (long l1 : operands) {
         for (long l2 : operands) {
            ...
            for (long l3 : operands) {
               ...
               long res = int128_udivideBy64(product2[0], product2[1], l3);
               long res2 = expectedResult.divide(bigIntOfUnsigned(l3)).longValue();

               if (res != res2) {
                  System.err.printf("%x * %x / %x should be %x but was %x%n", l1, l2, l3,
                          res2, res);
               }
            }
         }
      ...
      for (long l1 : operands) {
         for (long l2 : operands) {
            ...
            if (!s.equals(s2)) {
               System.err.printf("%x %x as big integer resulted in \"%s\" instead of \"%s\"%n", l1, l2, s, s2);
            }
            for (long l3 : operands) {
               if (l3 == 0) {
                  continue;
               }
               long res = int128_udivideBy64(l1, l2, l3);
   ...
   private static BigInteger bigIntOfUnsigned(long l) {
      if (l < 0) {
